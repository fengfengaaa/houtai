<template>
    <div>
      ...
      <!--          <el-button
                ...
                <el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-if="hasPerm('table:add')">新增</el-button>
            </div>
            <el-table
                :data="tableData"
                border
                ...
                @row-dblclick="handleDbClickPatient"
            >
                ...
                <el-table-column label="操作" width="300" align="center">
                    <template slot-scope="scope">
                        ...
                        <el-button
                                ...
                                v-if="hasPerm('table:disableStatus') && scope.row.status == 'N'"
                        >禁用</el-button>
                        <el-button
                                type="text"
                                icon="el-icon-delete"
                                class="red"
                                @click="handleStatu(scope.$index, scope.row)"
                                v-if="hasPerm('table:enableStatus') && scope.row.status == 'D'"
                        >启用</el-button>
                        <el-button
                                type="text"
                                icon="el-icon-delete"
...
export default {
    ...
    data() {
        ...
        return {
            ...
            rules: {
                userName: [{ required: true, message: '请输入用户名', trigger: 'blur',validator: validateSpace}],
                password: [{ required: true, message: '请输入密码', trigger: 'blur',validator: validateSpace}],
                privilege: [{ required: true, message: '请选择权限', trigger: 'blur', validator: validateSpace }]
            },
            tableData: [],
            patientData: [],
            exportTableData: [],
    ...
    methods: {
        ...
        handleDbClickPatient(row){
            if(this.hasPerm('table:patient')){
                this.query.userName = row.userName;
                this.patientVisible = true;
                this.getPatientData();
            }
